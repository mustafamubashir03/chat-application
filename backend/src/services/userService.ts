...
export const signupService = async(data: UserSignUpType) => {
  try {
    ...
    if(ENABLE_EMAIL_VERIFICATION){
      ...
      const verificationUrl = APP_URL.replace(/^https:\/\/(localhost|127\.0\.0\.1)/i, 'http://$1');
      await addEmailtoMailQueue({...mailObject,to:newUser.email,subject:"Welcome to the app.Please verify your email",text:`Welcome to the app. Please verify your email by clicking on the link below:${verificationUrl}/verify/${newUser.verificationToken}`})
    }
    return newUser;
  } catch (error: any) {

...
const WorkspacePreferencesModal = () => {
  ...
  const { Confirmation, ConfirmDialog } = useConfirm({
    title: 'Are you sure that you want to delete the workspace?',
    message: 'This action cannot be undone',
  })
  const { Confirmation: updateConfirmation, ConfirmDialog: UpdateConfirmDialog } = useConfirm({
    title: 'Are you sure that you want to update the name of the workspace?',
    message: 'This action cannot be undone',
  })
              ...
              <DialogTrigger className="flex flex-col justify-start">
                <div className="flex flex-col px-5 py-4 bg-slate-800 rounded-lg w-full cursor-pointer hover:bg-blue-800/30">
                  <div className="flex items-start justify-between w-full text-slate-400">
                    ...
                    <p className="text-sm gap-2 flex font-semibold hover:underline">
                ...
                <form onSubmit={handleUpdateFormSubmit} className="space-y-4 text-slate-400">
                  ...
                  <DialogFooter>
                    <DialogClose className="w-full flex justify-end">
                      <Button
                        className="outline-slate-400 text-slate-400 "
                        disabled={isPending}
                        variant={'outline'}
                      >
                        Cancel

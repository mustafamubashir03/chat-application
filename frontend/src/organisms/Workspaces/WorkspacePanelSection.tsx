...
const WorkspacePanelSection = ({children,label,openState,setOpenState}:{children:React.ReactNode,label:string,openState:boolean,setOpenState:React.Dispatch<React.SetStateAction<boolean>>}) => {
    const {setOpenCreateChannelModal} = useCreateChannelModal()
  return (
    <div className="flex flex-col mt-2 px-2">
        <div  onClick={()=>setOpenState(!openState)} className="flex items-center px-2 group cursor-pointer transition-all">
            <Button 
                variant="transparent"
                size="sm"
                className="p-1 text-slate-400 hover:text-slate-300 transition-colors"
            >
                <ChevronDown className={`size-4 transition-transform ${openState ? 'rotate-0' : '-rotate-90'}`}/>
            </Button>

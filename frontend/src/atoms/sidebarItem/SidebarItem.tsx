...
const SidebarItem = ({label,Icon,variant}:{label:string,Icon:LucideIcon,variant:"default"|"active"}) => {
  const {workspaceId} = useParams()
    const {channelId} = useParams()

  return (
    <Button className={cn(sidebarItemVariants({variant}))} variant={"transparent"} size={"sm"}>
        <Link className="flex items-center justify-center gap-2" to={`/workspace/${workspaceId}/channels/${channelId}`}>
